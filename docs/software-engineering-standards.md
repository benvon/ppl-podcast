...
- Bind a report or approval to the current relevant bytes. Spoken narration, source-tag, claim, or cited-source changes invalidate source review; version metadata and show-notes-only changes do not. A pure show-notes mapping check runs before rendering and package-only validation; publication-day reachability binds the current show notes and manifest and checks every listed link. Editorial approval remains bound to exact script bytes. A changed audio file invalidates audio and chapter review; a changed handoff input invalidates the handoff.
