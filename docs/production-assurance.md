...
2. With explicit current-turn authorization, run `npm run sources:preflight -- --episode episodes/EPISODE --require-llm` to independently fetch the exact cited targets and send the extracted locator text to the LLM claim-source challenger. Resolve every unsupported, over-broad, or incomplete claim before the drafting agent writes full spoken prose. The tool records the input hashes, exact reviewed locators, independently fetched citation identity and content hash, locator-excerpt hash, claim mappings, and LLM assessments in `claim-source-preflight.yaml`; record findings and resolutions in `production-log.md`.
