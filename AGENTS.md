...
4. Use the current Sol model with high reasoning effort for the source-led research pass and first complete draft. It produces the source ledger, claim inventory, research packet, source-tagged `master-script.md`, derived `narration.md`, show notes, metadata, and production log. Complete these gates before presenting the draft for human editorial review:
   ...
   - Before formal source relevance, perform a source-contract self-audit of every source-tagged factual paragraph. Preserve the cited source’s actor, action, scope, condition, exception, and exact locator; split chart facts, regulatory requirements, and scenario synthesis into separately tagged statements; and recheck every Retrieval review recap for dropped conditions. Do not broaden a precise source noun, launder an exception from another authority into a generic “except” clause, or use a chart symbol alone to establish a regulatory obligation. This is required drafting discipline, not another validator or release gate.
   - With explicit current-turn authorization, run source-relevance validation with `--require-llm` against the source excerpts, claims, and tagged spoken passages. It runs two independent assessments against the same frozen inputs and must identify only source-support risks: contradiction, unsupported factual prose, incorrect locator, or material scope mismatch. A finding blocks source review only when it could teach an unsafe, wrong, or materially incomplete decision, rule, condition, or central lesson concept. Exact-but-nonessential legal wording is an editorial note, retained for the human editor but not an automatic rewrite or release blocker. Resolve the union of material findings as one contextual revision of each affected section, then rerun until no material findings remain before human editorial review; record any editorial decision in `production-log.md`, not a waiver system.
...
8. Draft as speech for a first-time listener. Use a scenario to carry one operational question through each major section: every source-backed fact should change what the pilot notices, compares, plans, or decides. Avoid slogans, quips, snark, clever reversals, and compressed regulatory labels. For every rule, explain the practical question first, then the evidence and terms that answer it. For regulatory episodes, use ACS knowledge and risk-management outcomes as the outline; teach only the detail needed for the typical private-pilot decision and link broader exceptions or specialized cases in show notes. Record stable pronunciation corrections only in the renderer's versioned pronunciation maps, with a focused test; use the existing `(RADIO)` speaker label and audio-mix configuration for local treatments. Do not create narration sidecars or alter spoken script text solely to direct pronunciation.
