...
7. Before handing off to hosting, run `npm run release:prehost -- --episode episodes/<episode-id-and-slug>`. It checks the approved MP3 against its render, chapter, and audio-quality records; checks the release metadata and source-validation record; and rejects duplicate public production notices.
