...
function validateClaimAssessments(relevance, expectedClaimIds) {
  ...
  const materialUnsupported = assessments.filter((assessment) => expected.has(assessment.claim_id) && assessment.verdict !== "supports" && assessment.finding_materiality !== "editorial").map((assessment) => assessment.claim_id);
  const editorialNotes = assessments.filter((assessment) => expected.has(assessment.claim_id) && assessment.verdict !== "supports" && assessment.finding_materiality === "editorial").map((assessment) => assessment.claim_id);
