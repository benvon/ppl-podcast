...
- Bind a report or approval to the current relevant bytes. Spoken narration, source-tag, claim, or cited-source changes invalidate source review; version metadata and show-notes-only changes do not. The deterministic public-link check binds current show notes and their manifest. Editorial approval remains bound to exact script bytes. A changed audio file invalidates audio and chapter review; a changed handoff input invalidates the handoff.
