...
test("pre-hosting validation requires consistent release records", () => {
  ...
  fs.writeFileSync(renderPath, JSON.stringify({ audio: { output: audioPath, sha256, duration_seconds: 60, quality: { result: "passed", report: qualityPath } }, chapters: { audio_sha256: sha256, markers: [{ start_ms: 0, title: "Opening" }] } }));
  fs.writeFileSync(qualityPath, JSON.stringify({ result: "passed", manifest: renderPath, output: { path: audioPath } })); fs.writeFileSync(reviewPath, `<meta name="ppl-audio-sha256" content="${sha256}">`);
  ...
  fs.writeFileSync(path.join(episodePath, "link-validation.yaml"), YAML.stringify({ checked_at_utc: "2026-08-24T13:32:00Z", show_notes_mapping: { valid: true }, show_notes_results: [{ source_id: "test-source", citation_target: { valid: true }, link: { valid: true } }], results: [{ source_id: "test-source", link: { valid: true }, relevance: { status: "assessed" }, claim_assessments: { valid: true } }] }));
  ...
  try {
    ...
    fs.writeFileSync(path.join(episodePath, "link-validation.yaml"), YAML.stringify({ checked_at_utc: "2026-08-24T13:32:00Z", show_notes_mapping: { valid: true }, show_notes_results: [{ source_id: "test-source", citation_target: { valid: true }, link: { valid: false } }], results: [{ source_id: "test-source", link: { valid: true }, relevance: { status: "assessed" }, claim_assessments: { valid: true } }] }));
    ...
    fs.writeFileSync(path.join(episodePath, "link-validation.yaml"), YAML.stringify({ checked_at_utc: "2026-08-24T13:32:00Z", show_notes_mapping: { valid: true }, show_notes_results: [{ source_id: "test-source", citation_target: { valid: true }, link: { valid: true } }], results: [{ source_id: "test-source", link: { valid: true }, relevance: { status: "assessed" }, claim_assessments: { valid: true } }] }));
    fs.writeFileSync(qualityPath, JSON.stringify({ result: "passed", manifest: path.join(temporary, "different.render-manifest.json"), output: { path: audioPath } }));
    ...
    fs.writeFileSync(qualityPath, JSON.stringify({ result: "passed", manifest: renderPath, output: { path: audioPath } }));
