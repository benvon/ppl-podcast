...
The executable registry in `scripts/episode-package-lifecycle.cjs` is the complete current-contract package-operation inventory: claim-source preflight, formal source review, script reset, script approval, Realtime rendering, pre-hosting validation, publication preparation, and hosting handoff construction. A command must acquire its named registry operation; it may not construct an ad hoc package lease. The registry records operation identity in each new lease, permits stale recovery only by the interrupted operation, and has matrix coverage for every operation. Locks created before operation identities existed remain recoverable only by the exact formerly recorded validator, so an interrupted current package is not stranded while cross-operation recovery remains blocked.

Lease publication is atomic: a complete, fsynced record is prepared beside the final path and claimed without replacement. Recovery itself uses the same owner-recorded, recoverable lease pattern. An explicit recovery may archive a malformed lock from the older publisher only after a bounded publication grace period; it never treats a freshly incomplete record as stale. Multi-file state transactions recheck each original target immediately before promotion and never roll back a file that changed after the transaction wrote it.
