...
- Record pronunciation-sensitive terms and intentionally treated turns in the narration plan before rendering. Render an opening/pronunciation-risk preview before the full episode, including any new local audio treatment or music behavior. Reuse a segment only if its written input, voice, and renderer settings match the current narration plan.
