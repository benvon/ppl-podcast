...
const DEFAULTS = {
  ...
  stitchFadeMs: 8,
  musicBedGainDb: -24,
  musicIntroLeadSeconds: 7,
  musicIntroTailSeconds: 2,
...
function mixMusicBeds({ voiceMasterPath, outputPath, music, plan }) {
  ...
  const addBed = (cue, fadeStartSeconds = null) => {
    ...
    filters.push(`[${inputIndex}:a]aformat=sample_rates=24000:channel_layouts=mono,volume=${music.gainDb}dB,atrim=duration=${cue.duration_seconds}${fade},adelay=${Math.round(cue.start_seconds * 1000)}[music_${inputIndex}]`);
  ...
  filters.push("[music][voice_sidechain]sidechaincompress=threshold=0.02:ratio=12:attack=5:release=500[ducked_music]");
  filters.push("[voice_mix][ducked_music]amix=inputs=2:duration=longest:normalize=0,alimiter=limit=0.89:level=disabled[mixed]");
...
async function main() {
  ...
  const musicValuesSpecified = ["music-bed-gain-db", "music-intro-lead-seconds", "music-intro-tail-seconds", "music-intro-fade-seconds", "music-outro-tail-seconds", "music-outro-fade-seconds"].some((name) => raw[name] !== undefined);
