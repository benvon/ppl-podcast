...
- The full 25:51 candidate was assembled from the accepted opening preview plus the authorized remaining segments. Automated WAV/MP3 decode, 24 kHz mono format, duration agreement, stitch-boundary, clipping, and ffprobe chapter checks passed. Full script-aligned listening QA and the twelve embedded chapter markers were accepted; the checksum-bound render manifest, quality report, and chapter-review page are recorded in `audio-manifest.yaml`.
- The deterministic publication-day link check completed successfully on 2026-09-22 for all 16 ledger sources and all 17 show-notes links. Its structured evidence is `publication-link-validation.yaml`; this does not replace the formal source-relevance record.
