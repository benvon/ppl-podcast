...
- `production-log.md` may explain decisions and editorial context, but it is never an audit input. Do not place an authoritative status, timestamp, run ID, hash, or report result there; point to the structured record that owns that fact instead.
...
5. Obtain human editorial approval. The claim inventory, source ledger, and source-tagged spoken script own substantive facts; show notes derive a listener-facing study guide from them and may include supplemental links without claim associations. If spoken narration, claims, cited source evidence, or source tags change, repeat source relevance before rendering. A show-notes-only change needs deterministic mapping checks before rendering and package-only validation, then current link reachability checks on publication day; it does not need a new OpenAI assessment. Formal source review identity follows the claims, cited sources, narration, and its source tags rather than show notes, version metadata, or incidental whitespace. Exact script bytes still govern editorial approval and rendered artifacts.
...
- The renderer's versioned `PRONUNCIATION_TRANSFORMS` and `PRONUNCIATION_GUIDANCE` maps in `scripts/render_episode_realtime.cjs` are the authoritative pronunciation record. Add a stable phonetic correction there with a focused renderer test; its contents are copied into the render settings and candidate manifest. Keep the written script unchanged for pronunciation alone. If a need cannot be expressed as a transform or silent guidance, propose a renderer enhancement rather than creating an episode sidecar. Intentional VHF treatment is recorded by the existing `(RADIO)` speaker label, and music behavior by the existing audio-mix configuration.
- Render an opening/pronunciation-risk preview before the full episode, including any new local audio treatment or music behavior. Reuse a segment only if its written input, voice, and renderer settings match the current narration plan.
...
| Show-notes mapping | A pure check binds every declared claim-backed or supplemental link to the current notes before rendering and package-only validation. |
| Public links | The publication-day report checks every current show-notes link, including supplemental study links, against the current notes and manifest. |
