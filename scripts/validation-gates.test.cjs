...
test("pre-hosting validation requires consistent release records", () => {
  ...
  const renderRecord = (chapterMarkers = markers) => ({ audio: { output: audioPath, sha256, duration_seconds: 2, quality: { result: "passed", report: qualityPath } }, chapters: { audio_sha256: sha256, markers: chapterMarkers } });
  fs.writeFileSync(renderPath, JSON.stringify(renderRecord()));
  fs.writeFileSync(qualityPath, JSON.stringify({ result: "passed", manifest: renderPath, output: { path: audioPath, sha256 } })); fs.writeFileSync(reviewPath, `<meta name="ppl-audio-sha256" content="${sha256}">`);
  ...
  fs.writeFileSync(path.join(episodePath, "audio-manifest.yaml"), YAML.stringify({ status: "candidate_rendered_listening_qa_approved", current_candidate_render: { sha256, duration_seconds: 2, mp3: path.basename(audioPath), render_manifest: path.basename(renderPath), audio_quality_report: path.basename(qualityPath), chapter_review: path.basename(reviewPath), validation: "passed" }, chapter_markers: { status: "embedded_and_ffprobe_validated", audio_sha256: sha256 } }));
  ...
  try {
    ...
    fs.writeFileSync(renderPath, JSON.stringify(renderRecord()));
    fs.writeFileSync(qualityPath, JSON.stringify({ result: "passed", manifest: path.join(temporary, "different.render-manifest.json"), output: { path: audioPath, sha256 } }));
    ...
    fs.writeFileSync(qualityPath, JSON.stringify({ result: "passed", manifest: renderPath, output: { path: audioPath, sha256: "b".repeat(64) } }));
    ...
    fs.writeFileSync(qualityPath, JSON.stringify({ result: "passed", manifest: renderPath, output: { path: audioPath, sha256 } }));
