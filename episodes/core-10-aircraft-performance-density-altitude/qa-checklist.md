...
- [x] The focused airspeed discussion defines only the four values needed for performance planning; a scheduled supplemental retains the deeper airspeed lesson.
...
- [x] Before opening an episode PR, approved-draft package validation passes with `npm run release:prehost -- --episode <episode-directory> --package-only`.
