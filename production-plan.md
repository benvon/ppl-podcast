...
5. Before formal source relevance, the drafting agent performs a source-contract self-audit of every source-tagged factual paragraph. It must preserve each source’s actor, action, scope, condition, exception, and exact locator; split chart facts, regulatory requirements, and scenario synthesis into separately tagged statements. It must also recheck every Retrieval review recap for conditions that could have been dropped while shortening the explanation. This is drafting discipline, not a new release gate or validator.
